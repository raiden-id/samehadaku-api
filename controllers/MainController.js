...
class MainController {
  home({ params: { page } }, req) {
    page =
      typeof page === "undefined"
        ...
        ? ""
        : `page/${page.toString()}/`;
    scraperjs.StaticScraper.create(`https://samehadaku.rent/${page}`).scrape(
      async ($) => {
        const obj = {};
        obj.season = $(".animposx")
          .map(function () {
            return {
              ...
              linkId: $(this)
                .find("a")
                .attr("href")
                .replace("https://samehadaku.rent/anime/", "")
                .replace("/", ""),
              image: $(this)
  ...
  blog({ params: { page } }, req) {
    ...
    scraperjs.StaticScraper.create(
      `https://samehadaku.rent/blog/${page}`
    ).scrape(($) => {
      const blog = {};
      blog.blog = $(".box-blog")
        .map(function () {
          return {
            ...
            linkId: $(this)
              .find(".img a")
              .attr("href")
              .replace("https://samehadaku.rent/blog/", "")
              .replace("/", "")
              .trim(),
  ...
  readblog({ params: { id } }, req) {
    const page = `https://samehadaku.rent/blog/${id}`;
  ...
  tag({ params: { tag } }, req) {
    const page = `https://samehadaku.rent/tag/${tag}`;
  ...
  blogcategory({ params: { category } }, req) {
    const page = `https://samehadaku.rent/blog-category/${category}`;
  ...
  blogCategoryByPage({ params: { category, page } }, req) {
    const pager = `https://samehadaku.rent/blog-category/${category}/page/${page}`;
  ...
  daftarGenre(_, req) {
    scraperjs.StaticScraper.create("https://samehadaku.rent/").scrape(($) => {
      const obj = {};
      obj.daftar_genere = $(".genre > li")
        .map(function () {
          ...
          return {
            ...
            linkid: $(this)
              .find("a")
              .attr("href")
              .replace("https://samehadaku.rent/genre/", ""),
            total: span,
          };
        })
        .get();
  ...
  anime({ params: { id } }, req) {
    const page = `https://samehadaku.rent/anime/${id}/`;
    scraperjs.StaticScraper.create(page).scrape(async ($) => {
      ...
      await scraperjs.StaticScraper.create("https://samehadaku.rent/").scrape(
        async ($) => {
  ...
  async readanime({ params: { link } }, req) {
    const page = `https://samehadaku.rent/${link}/`;
    console.log(page);
  ...
  search({ params: { title } }, req) {
    const page = `https://samehadaku.rent/?s=${title}`;
  ...
  searchByPage({ params: { title, page } }, req) {
    const pager = `https://samehadaku.rent/page/${page}/?s=${title}`;
  ...
  season(_, req) {
    const page = "https://samehadaku.rent/season/spring-2020/";
    ...
    scraperjs.StaticScraper.create(page)
      ...
      .then((data) => {
        const page = "https://samehadaku.rent/season/spring-2020/page/2/";
  ...
  date(_, req) {
    const page = "https://samehadaku.rent/jadwal-rilis/";
  ...
  listWithoutPage(_, req) {
    const page = "https://samehadaku.rent/daftar-anime/";
  ...
  listWithPage({ params: { page } }, req) {
    const inPage = `https://samehadaku.rent/daftar-anime/page/${page}/`;
  ...
  searchByGenre({ params: { genre } }, req) {
    const page = `https://samehadaku.rent/genre/${genre}`;
